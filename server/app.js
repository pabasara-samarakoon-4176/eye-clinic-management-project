...
module.exports = function (db) {
    ...
    app.get('/getdoctor', async (req, res) => {
        ...
        const result = await db.getDoctor(doctorId)
        res.send(result)
    })
    ...
    app.post("/login", async (req, res) => {
        ...
        try {
            const loginStatus = await db.login(doctorId, password)
    ...
    app.post("/register", async (req, res) => {
        ...
        try {
            ...
            const message = await db.register(doctorId, doctorFirstname, doctorLastname, doctorPassword)
            res.status(200).json({
                message: message
            })
    ...
    app.get("/searchpatient/:patientId", async (req, res) => {
        const patientId = req.params.patientId
        try {
            const response = await db.searchPatient(patientId)
            res.send(response)
        } catch (error) {
            console.log(`${error.message}`)
        }
    ...
    app.post("/addlens/:nurseId", async (req, res) => {
        ...
        try {
            const adminId = 'MBBS.00000'
            const nurseId = req.params.nurseId;
            const response = await db.addLens(lensType, manufacturerId, 
                surgeryType, model, lensPower, manufactureDate, expiryDate, batchNo, remarks, adminId, nurseId)
            res.send(response)
        } catch (error) {
            if (error.code === 'ER_DUP_ENTRY') {
                res.json({
    ...
    app.get("/admin/viewappointmentdetails/:patientId", async (req, res) => {
        const patientId = req.params.patientId
        try {
            const response = await db.viewappointmentdetails(patientId)
            res.send(response)
        } catch (error) {
            console.log(error)
        }
